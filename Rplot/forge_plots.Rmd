...
```{r read_forge}
read_forge <- function (fn, series, blowup, mem=T) {
  ...
  if (mem) {
    tab <- ddply(tab, .(Pct), summarize, Hap="All", Aligned=mean(Aligned), Correct=mean(Correct), Overall=mean(Overall), AlignMem=mean(AlignMem), BuildMem=mean(BuildMem))
  }
  else {
    tab <- ddply(tab, .(Pct), summarize, Hap="All", Aligned=mean(Aligned), Correct=mean(Correct), Overall=mean(Overall))
  }

  tab <- cbind(tab, list('series'=series, 'blowup'=blowup))
  tab$incorrect <- tab$Aligned - tab$Overall
  tab$cor_minus_incor <- tab$Overall - tab$incorrect
...
read_forge_join <- function (fn1, fn2, series, blowup) {
  ...
  tab <- dplyr::inner_join(tab1, tab2, by='Pct')
  tab <- ddply(tab, .(Pct, Num), summarize, Hap="All", Aligned=mean(Aligned), Correct=mean(Correct), Overall=mean(Overall), AlignMem=mean(AlignMem), BuildMem=mean(BuildMem))
  tab <- cbind(tab, list('series'=series, 'blowup'=blowup))
  tab$incorrect <- tab$Aligned - tab$Overall
  tab$cor_minus_incor <- tab$Overall - tab$incorrect
  tab
...
```{r chr9_hisat}
plot_fig1 <- function(m, groups=list('series', 'blowup'), hap='All', show_legend=T, show_rand=T) {
  #if (show_rand) {
    myColors <- brewer.pal(6, "Set1")
    names(myColors) <- c('Population coverage', 'Hybrid', 'Random', 'Personalized', 'HISAT2 Auto', 'Major Allele')
  #} else {
  ...
  curve_helper <- function(m, x_column_name, x_label, y_column_name, y_label) {
    ...
    m <- m %>% filter(Pct != 'Auto' && Pct != 'Major' && Pct != 'NA12878')
...
```{r figpaired}
m <- rbind(read_forge_join('../results/chr9_hisat_popcov_paired_ind.tsv',
                           '../results/chr9_hisat_popcov_paired_mem.tsv',
                           'Population coverage', 'Standard'),
           read_forge_join('../results/chr9_hisat_popcov_blowup_paired_ind.tsv',
                           '../results/chr9_hisat_popcov_blowup_paired_mem.tsv',
                           'Population coverage', 'Blowup avoid'),
           read_forge_join('../results/chr9_hisat_hybrid_paired_ind.tsv',
                           '../results/chr9_hisat_hybrid_paired_mem.tsv',
                           'Hybrid', 'Standard'),
           read_forge_join('../results/chr9_hisat_hybrid_blowup_paired_ind.tsv',
                           '../results/chr9_hisat_hybrid_blowup_paired_mem.tsv',
                           'Hybrid',  'Blowup avoid'))

auto <- read_forge('../results/chr9_hisat_auto_paired_ind.tsv', 'HISAT2 Auto', 'Standard')
auto$Num <- auto$Pct

major <- read_forge('../results/chr9_major_ref_paired_ind.tsv', 'Major Allele', 'Standard')
major$Num <- major$Pct

indiv <- read_forge('../results/chr9_NA12878_ref_paired_ind.tsv', 'Personalized', 'Standard')
indiv$Num <- indiv$Pct
...
plot_fig1(m, show_rand=F)

pdf(file='fig1_paired.pdf', onefile=F, width=9, height=6)
plot_fig1(m, show_rand=F)
dev.off()
```
...
```{r figYRI}
m <- rbind(read_forge_join('../results/chr9_hisat_popcov_NA19238.tsv',
                           '../results/chr9_hisat_popcov_NA19238_mem.tsv',
                           'Population coverage', 'Standard'),
           read_forge_join('../results/chr9_hisat_popcov_blowup_NA19238.tsv',
                           '../results/chr9_hisat_popcov_blowup_NA19238_mem.tsv',
                           'Population coverage', 'Blowup avoid'),
           read_forge_join('../results/chr9_hisat_hybrid_NA19238.tsv',
                           '../results/chr9_hisat_hybrid_NA19238_mem.tsv',
                           'Hybrid', 'Standard'),
           read_forge_join('../results/chr9_hisat_hybrid_blowup_NA19238.tsv',
                           '../results/chr9_hisat_hybrid_blowup_NA19238_mem.tsv',
                           'Hybrid',  'Blowup avoid'))

auto <- read_forge('../results/chr9_NA19238_hisat_auto.tsv', 'HISAT2 Auto', 'Standard')
auto$Num <- auto$Pct

major <- read_forge('../results/chr9_NA19238_major_ref.tsv', 'Major Allele', 'Standard')
major$Num <- major$Pct

indiv <- read_forge('../results/chr9_NA19238_ref.tsv', 'Personalized', 'Standard')
indiv$Num <- indiv$Pct
...
pdf(file='fig1_YRI.pdf', onefile=F, width=9, height=6)
plot_fig1(m, show_rand=F)
dev.off()
```
...
m <- rbind(read_forge_join('../results/chr9_hisat_popcov.tsv',
          ...
          #                 'Hybrid', 'Standard'),
           #read_forge_join('../results/chr9_hisat_amb_blowup.tsv',
            ...
            #               'Hybrid',  'Blowup avoid'),
           read_forge_join('../results/chr9_hisat_hybrid.tsv',
                           '../results/chr9_hisat_amb_mem.tsv',
                           'Hybrid', 'Standard'),
           read_forge_join('../results/chr9_hisat_hybrid_blowup.tsv',
                           '../results/chr9_hisat_amb_blowup_mem.tsv',
                           'Hybrid',  'Blowup avoid'),
           read_forge_join('../results/chr9_hisat_rand.tsv',
                           '../results/chr9_hisat_rand_mem.tsv',
                           'Random',  'Standard'))
...
pdf(file='fig1_new.pdf', onefile=F, width=9, height=6)
plot_fig1(m)
dev.off()
```
...
```{r chr9_hisat_strat}
plot_fig_strat <- function(m1, m2, m3, groups=list('series', 'strat'), hap='All', show_legend=T) {
  myColors <- brewer.pal(5, "Set1")
...
```{r read_forge_strat}
read_forge_strat <- function (fn, strat_mode, series, blowup) {
  tab <- read.csv(fn, sep='\t', comment.char='#')
  tab$strat <- tab[[strat_mode]]
  tab <- ddply(tab, .(Pct, strat), summarize, Hap="All", Aligned=mean(Aligned), Correct=mean(Correct), Overall=mean(Overall), Total=mean(Total))
  tab <- cbind(tab, list('series'=series, 'blowup'=blowup))
  tab$incorrect <- tab$Aligned - tab$Overall
  tab$cor_minus_incor <- tab$Overall - tab$incorrect
...
```{r fig3}
m1 <- rbind(read_forge_strat('../results/chr9_hisat_popcov.safe.strat_snp.tsv', 'SNPs', 'Population coverage', 'Standard'))
...
m2 <- rbind(read_forge_strat('../results/chr9_hisat_popcov.safe.strat_rare.tsv', 'RareSNPs', 'Population coverage', 'Standard'))
...
m3 <- rbind(read_forge_strat('../results/chr9_hisat_popcov.safe.strat_region.tsv', 'Region', 'Population coverage', 'Standard'))
...
```{r chr19_sim}
plot_fig4 <- function(m, groups=list('series', 'blowup'), x_column_name='Pct', hap='All', x_label='% SNVs', show_legend=T) {
  myColors <- brewer.pal(5, "Set1")
  names(myColors) <- c('Population coverage', 'Hybrid', 'Personalized', 'HISAT2 Auto', 'Major Allele')
...
#                      'Hybrid', 'CEU'))
m <- rbind(read_forge_join('../results/chr9_hisat_popcov_blowup.tsv', '../results/chr9_hisat_popcov_blowup_mem.tsv',
                           'Population coverage', 'All'),
           read_forge_join('../results/ceu/chr9_ceu_popcov_blowup_l100.tsv', '../results/ceu/chr9_ceu_counts.tsv', 
                      'Population coverage', 'CEU'),
           read_forge_join('../results/chr9_hisat_amb.tsv', '../results/chr9_hisat_amb_mem.tsv',
                      'Hybrid', 'All'),
           read_forge_join('../results/ceu/chr9_ceu_amb_l100.tsv', '../results/ceu/chr9_ceu_counts.tsv', 
...
plot_full_exp <- function(up, bias, groups=list('series')) {
  myColors <- brewer.pal(5, "Set1")
  names(myColors) <- c('SNVs Only', 'SNVs + Indels', 'HISAT2 Auto')
  ...
  unique_perfect <- function(up) {
    ...
    full_pct1 <- filter(filter(up, Pct == 70), series == 'SNVs + Indels')
    full_pct2 <- filter(filter(up, Pct == 80), series == 'SNVs Only')
    ...
    ggplot(up) +
      ...
      geom_point(data=zero_pct, aes(x=Unique, y=Perfect),
                 shape=16, fill='black', color='black', size=3) +
      geom_point(data=full_pct1, aes(x=Unique, y=Perfect),
                 shape=15, fill='black', color='black', size=3) +
      geom_point(data=full_pct2, aes(x=Unique, y=Perfect),
                 shape=15, fill='black', color='black', size=3) +
      # labels for optimum points
      geom_text_repel(data=filter(mg, (pct != 'Auto')), aes(x=x, y=y, label=pct, color=series),
                      segment.color='black', point.padding=0.7,
                      min.segment.length = unit(0, 'lines')) +
      # labels for 0/80% points
      geom_text_repel(data=zero_pct, aes(x=Unique, y=Perfect, label='0%')) +
      geom_text_repel(data=full_pct1, aes(x=Unique, y=Perfect, label='70%')) +
      geom_text_repel(data=full_pct2, aes(x=Unique, y=Perfect, label='80%')) +
...
```{r fig5}
sim <- read_forge('../results/full_genome/popcov_blowup_sim.tsv', 'Population coverage', 'Blowup Avoid', mem=F)
#sim <- read_forge('../results/chr9_vs_full.tsv', 'Population coverage', 'Blowup Avoid', mem=F)

indiv <- read_forge('../results/full_genome/NA12878_sim.tsv', 'Personalized', 'Standard', mem=F)
#indiv <- read_forge('../results/chr9_vs_full_NA12878.tsv', 'Personalized', 'Standard', mem=F)
#indiv$Num <- indiv$Pct
...
#pdf(file='chr9_vs_full.pdf', onefile=F, width=9, height=4)
plot_full_sim(sim)
dev.off()
```
...
up_auto <- read_unique_perf('../results/full_genome/unique_perfect_hisat_auto.tsv')
up_auto$series = 'HISAT2 Auto'

up <- rbind(up_vars, up_snps, up_auto)
...
```{r chr9_unique_perfect}

up <- read_unique_perf('../results/chr9_unique_perfect_popcov.tsv')
...
unique_perfect <- function(up) {
    ...
    up <- up %>% arrange(Pct)
    zero_pct <- filter(up, Pct == 0)[1,]
    full_pct <- filter(up, Pct == 100)
    #horizontal <- as.character(up$Pct)
    
    ggplot(up) +
      geom_path(aes(x=Unique, y=Perfect, color=series)) +
      # optimal points
      ...
      # labels for 0/80% points
      geom_text_repel(data=zero_pct, aes(x=Unique, y=Perfect, label='0%')) +
      geom_text_repel(data=full_pct, aes(x=Unique, y=Perfect, label='100%')) +
      labs(x='Unique Mappings (Billions)', y='Perfect Mappings (Billions)') +
      scale_color_manual(name='series', values=myColors, drop=F) +
      scale_fill_manual(name='series', values=myColors, drop=F) +
      theme_bw() + theme(legend.title=element_blank())
...
unique_perfect(up)
pdf(file='chr9_up.pdf', onefile=F)
unique_perfect(up)
dev.off()
```

```{r supp_fig}
plot_chr9_vs_full <- function(m, groups=list('series', 'blowup'), hap='All', show_legend=T) {
  myColors <- brewer.pal(5, "Set1")
  names(myColors) <- c('Population coverage', 'Hybrid', 'Personalized', 'HISAT2 Auto', 'Major Allele')
  ...
  parametric_helper <- function(m, x_column_name) {
    ...
    ggplot(m) +
      ...
      #coord_cartesian(ylim=c(91.1,91.7), xlim = c(5.34, 5.49)) +
      # labels for optimum points
      geom_text_repel(data=filter(mg, (pct != 'Auto' && pct != 'Major')), aes(x=x, y=y, label=blowup, color=series),
                      segment.color='black', point.padding=0.7,
                      min.segment.length = unit(0, 'lines')) +
      # labels for 0/100% points
      geom_text_repel(data=zero_pct, aes(x=incorrect, y=Overall, label='0%')) +
  ...
  parametric_helper(m, 'Pct')
}
```

```{r supp_fig1}
m <- rbind(read_forge('../results/chr9_vs_full/chr9_hisat_popcov.tsv',
                           'Population coverage', 'Chr9 SNVs', mem=F),
           read_forge('../results/chr9_vs_full/chr9_hisat_hybrid.tsv',
                           'Hybrid', 'Chr9 SNVs', mem=F))

m2 <- read_forge('../results/chr9_vs_full/chr9_vs_full.tsv', 'Population coverage', 'All SNVs', mem=F)

indiv <- read_forge('../results/chr9_vs_full/chr9_vs_full_NA12878.tsv', 'Personalized', 'All SNVs', mem=F)
...
pdf(file='supp_fig1.pdf', onefile=F, width=9, height=7)
plot_chr9_vs_full(m)
dev.off()
```

```{r Build mem}
m <- read.csv('../results/full_genome/build_mem_snps.tsv', sep='\t')

plot_mem <- function(m) {
  ggplot(m, aes(x=Pct, y=BuildMem)) + 
    geom_line() +
    theme_bw() + theme(legend.title=element_blank())
}

plot_mem(m)
pdf(file='build_mem.pdf', onefile=F, width=9, height=7)
plot_mem(m)
dev.off()
```
