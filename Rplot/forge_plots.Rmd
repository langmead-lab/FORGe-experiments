...
```{r full_genome}
plot_full <- function(sim, up, groups=list('series')) {
  curve_helper <- function(m, x_column_name, x_label, y_column_name, y_label) {
    m$vertical <- m[[y_column_name]]  # trick so I can use "varticle" bareword below
    # following line construct data frame for just the optimal points
    mg <- m %>% dplyr::group_by_(.dots = groups) %>%
  ...
  grid_arrange_shared_legend(T, 2, 4, p_ra, p_ca, p_ci, p_up, p_up, p_up)
}
```
...
if (F) {
  threshold <- "35"
  pcts <- c("0", "2", "4", "6", "8", "10", "15", "20", "30", "40", "50", "60", "70", "_auto")
  for (pct in pcts) {
    res <- read_dir('/Users/jacobpritt/Genomics/vis-experiments/results/allele_bias/threshold25_vars0')
    res$Pct <- pct
  }
}
```
