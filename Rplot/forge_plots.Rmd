...
plot_fig1 <- function(m, groups=list('series', 'blowup'), hap='A', show_legend=T) {
  ...
  curve_helper_mem <- function(m, x_column_name, x_label, y_column_name, y_label) {
    ...
    ggplot(m, aes(x=vertical, y=horizontal, color=get(groups[[1]]), linetype=get(groups[[2]]))) +
      geom_line() +
      geom_point(data=mg, aes(x=x, y=y, fill=get(groups[[1]])), shape=23, color='black', size=3, alpha=0.5) +
      labs(x=y_label, y=x_label) +
      theme_bw() + theme(legend.title=element_blank())
  }
...
pdf(file='fig4.pdf', onefile=F, width=7, height=3)
plot_fig4(m, x_column_name='Num', x_label='Thousands SNPs')
dev.off()
```
...
plot_full <- function(sim, up, groups=list('series')) {
  ...
  unique_perfect <- function(m) {
    ...
    ggplot(m) +
        ...
        geom_point(data=auto, aes(x=Unique, y=Perfect), shape=23, color='black', size=3, alpha=0.5) +
  ...
  grid_arrange_shared_legend(T, 2, 3, p_ra, p_ca, p_up, p_up)
}
```
