...
grid_arrange_shared_legend <- function(show_legend, ...) {
    plots <- list(...)
    if (show_legend) {
...
```{r chr19_sim}
plot_forge <- function(m, groups=list('series', 'blowup'), x_column_name='Pct', hap='A', plot_parametric=T, x_label='% SNPs', show_legend=T) {
  ...
  grid_arrange_shared_legend(show_legend, p_ra, p_ca, p_co, p_ci)
}
```
...
plot_forge_strat <- function(m, x_column_name='Pct', hap='A', plot_parametric=T, x_label='% SNPs') {
  ...
  grid_arrange_shared_legend(T, p_ra, p_ca, p_co, p_ci)
}
```
