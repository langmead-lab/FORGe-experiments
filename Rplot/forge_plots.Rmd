...
plot_fig_strat <- function(m1, m2, m3, groups=list('series', 'strat'), hap='A', show_legend=T) {
  ...
  parametric_helper <- function(m, x_column_name, snps=F) {
    ...
    pl <- ggplot(m) +
      ...
      theme_bw() + theme(legend.title=element_blank(), axis.text.x = element_text(angle = 30, hjust = 1))
    if(snps) {
        pl + coord_cartesian(xlim = c(0.125, 1.01), ylim=c(85, 98))
    } else {
        pl
    }
  }
...
m3 <- rbind(read_forge('../results/strat_all/chr9_all_popcov_safe.strat_region.tsv', 'Population coverage', 'Standard'))
...
```{r chr19_sim}
plot_fig4 <- function(m, groups=list('series', 'blowup'), x_column_name='Pct', hap='A', plot_parametric=T, x_label='% SNVs', show_legend=T) {
  m$horizontal <- m[[x_column_name]]  # trick so I can use "varticle" bareword below
  # TODO: add the haplotypes
  m <- m %>% filter(Hap == hap)
  ...
  if(plot_parametric) {
    ...
    p_ci <- curve_helper_mem(m, 'Pct', '% SNVs')
  }
...
plot_full <- function(sim, up, bias, groups=list('series')) {
  ...
  #grid_arrange_custom_layout(lay, 4, p_up, p_up, p_up, p_up, p_bi)
...
#levels(sim$series) <- cbind(levels(sim$series), c("Auto"))
sim$series <- as.character(sim$series)
#sim$series <- factor(sim$series, levels=c("Population Coverage", "Auto"))
...
pdf(file='fig5.pdf', onefile=F, width=9, height=6)
plot_full(sim, up, bias)
dev.off()
```
